...
def noname(query) : # db에 접속해서 쿼리문만으로 테이블명과 db_id를 가져와서 쿼리 난이도를 판별하는 함수
    # 티베로 db 접속
    ...
    # schemas 형태
    schemas = {db_id : {table_name.lower() : cols , table2_name.lower() : cols2}} # <- table with alie 대문자,소문자 차이는 여기서 보정
    tables = { db_id  : { 'column_names_original' : column_names_original, "table_names_original" : table_names_original2}}
...
noname("SELECT T1.year, T1.creASe_or_decreASe_overall_industrial_exports FROM IMPORT_EXPORT_STS_MOTIE_TEXTILE_I AS T1 JOIN CUR_STA_WORKERS_MOTIE_TEXTILE_I AS T2 ON T1.year = T2.year WHERE T1.year IN (SELECT year FROM IMPORT_EXPORT_STS_MOTIE_TEXTILE_I WHERE tal_industrial_income_amount > 100000) AND T2.number_textile_industry_businesses > 50000")
